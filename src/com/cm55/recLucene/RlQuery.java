...
public abstract class RlQuery {
   ...
   */
  public abstract <T> Query getLuceneQuery(RlFieldSet<T> fieldmap);
  ...
  public static class Match extends SingleValue  {
    ...
    /** Lucene用Queryを取得する */
    @Override
    public <T>Query getLuceneQuery(RlFieldSet<T> fieldmap) {    
  ...
  public abstract static class Compound<T extends Compound<T>> extends RlQuery {
    ...
    /** Lucene用のQueryを取得する */
    @Override
    public <S> Query getLuceneQuery(RlFieldSet<S> fieldmap) {
      BooleanQuery.Builder builder = new BooleanQuery.Builder();
